...
const headingTextClass = computed(() => 
  currentTheme.value === 'light' ? 'text-gray-800' : 'text-white'
)
...
const statValueTextClass = computed(() => 
  currentTheme.value === 'light' ? 'text-gray-800' : 'text-white'
)
...
<template>
  <div>
    <h1 class="text-2xl font-semibold flex items-center" :class="headingTextClass">
      <i class="fas fa-tachometer-alt mr-2 text-neon-blue"></i>
      仪表盘
    </h1>
    <p class="mt-1 text-sm" :class="subtitleTextClass">
    ...
    <div class="mt-6">
      <div class="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <div
          ...
          :class="[
            `border-${item.color}`,
            currentTheme === 'light' ? 'glass-light' : 'glass-dark'
          ]"
            ...
            <div class="flex items-center">
              <div class="flex-shrink-0">
                <div 
                  ...
                  :class="[
                    `text-${item.color}`,
                    currentTheme === 'light' ? 'stat-icon-light' : 'stat-icon-dark'
                  ]"
              ...
              <div class="ml-5 w-0 flex-1">
                <dl>
                  <dt class="text-sm font-medium truncate" :class="statLabelTextClass">
                    {{ item.name }}
                  </dt>
                  <dd>
                    <div class="text-2xl font-medium" :class="statValueTextClass">
                      {{ item.value }}
                    </div>
                  </dd>
                </dl>
    ...
    <div class="mt-8">
      <h2 class="text-lg font-medium flex items-center" :class="headingTextClass">
        <i class="fas fa-history mr-2 text-neon-purple"></i>
        最近活动
      </h2>
      <div class="mt-4 rounded-lg" :class="currentTheme === 'light' ? 'glass-light' : 'glass-morphism'">
        <div class="p-6">
          <div class="text-center py-8" :class="subtitleTextClass" v-if="userStore.loading">
            <i class="fas fa-spinner fa-spin mr-2"></i>
            加载中...
          </div>
          <div class="text-center py-8" :class="subtitleTextClass" v-else>
